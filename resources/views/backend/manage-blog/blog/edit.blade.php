                            ...
                            <div class="col-md-12">
                                ...
                                <table class="table align-middle mb-0 table-hover table-centered">
                                    ...
                                    <tr class="paragraphstr">
                                        ...
                                        <td>
                                            @php
                                            $productLinks = $paragraph->productLinks;
                                            $linkCount = $productLinks->count();
                                            @endphp

                                            {{-- Always show exactly 2 product link fields --}}
                                            @for ($linkIndex = 0; $linkIndex < 2; $linkIndex++)
                                                <label for="product_name_{{ $index }}_{{ $linkIndex }}" class="form-label">
                                                Select Blog Links {{ $linkIndex + 1 }}
                                                </label>
                                                <div class="position-relative autocompleted mb-2">
                                                    <div class="input-group">
                                                        <input type="text"
                                                            id="product_name_{{ $index }}_{{ $linkIndex }}"
                                                            name="product_name[{{ $index }}][products][{{ $linkIndex }}][name]"
                                                            class="form-control product-autocomplete"
                                                            autocomplete="off"
                                                            value="{{ $linkIndex < $linkCount ? $productLinks[$linkIndex]->links : '' }}">
                                                    ...
                                                    <input type="hidden"
                                                        name="product_id[{{ $index }}][products][{{ $linkIndex }}][id]"
                                                        class="product_id"
                                                        value="{{ $linkIndex < $linkCount ? $productLinks[$linkIndex]->product_id : '' }}">
                                                </div>
                                                @endfor
                                        </td>
                                        <td>
